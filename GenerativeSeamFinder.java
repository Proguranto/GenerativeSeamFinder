    ...
    private static class PixelGraph implements Graph<Node> {
        ...
        private final Node source = new Node() {
            ...
            public List<Edge<Node>> neighbors(Picture picture, EnergyFunction f) {
                ...
                List<Edge<Node>> returnList = new ArrayList<>(picture.height());
                for (int j = 0; j < picture.height(); j += 1) {
                    Pixel to = new Pixel(0, j);
                    returnList.add(new Edge<>(this, to, f.apply(picture, 0, j)));
                }
                return returnList;
            }
        };
        ...
        public class Pixel implements Node {
            ...
            public List<Edge<Node>> neighbors(Picture picture, EnergyFunction f) {
                // TODO: Replace with your code
                List<Edge<Node>> returnList = new ArrayList<>();
                // If this pixel is on the right-most edge
                if (x == picture.width() - 1) {
                    // Connect this pixel to the sink
                    returnList.add(new Edge<>(this, sink, 0));
                } else { // This pixel is not on the right-most edge
                    ...
                    for (int z = y - 1; z <= y + 1; z += 1) {
                        ...
                        if (0 <= z && z < picture.height()) {
                            ...
                            returnList.add(new Edge<>(this, to, f.apply(picture, x + 1, z)));
                        }
                    }
                }
                return returnList;
